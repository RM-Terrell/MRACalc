...
function deleteRow(row) {
    ...
    if (rowCount > 2) {
        ...
        (<HTMLTableElement>document.getElementById('InputTable')).deleteRow(rowCount - 1);
    }
...
function insRow() {
    var x = <HTMLTableElement>(document.getElementById('InputTable'));
    // copy the targeted row
    var new_row = <HTMLTableRowElement>(x.rows[1].cloneNode(true));
    // get the total number of rows
    var len = x.rows.length.toString();
    // set the innerHTML of the first row
    new_row.cells[0].innerHTML = len;

    var AppendTarget = <HTMLTableElement>(document.getElementById('AppendTarget'));
